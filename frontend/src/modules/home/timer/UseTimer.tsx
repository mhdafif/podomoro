...
const UseTimer = () => {
  ...
  const { activeTaskId, addTaskTime } = useTaskStore();
  ...
  const startNextSession = () => {
    ...
    // Mark current work session as finished and add time to active task
    if (currentStepInfo.session === "work") {
      ...
      // Add completed work time to active task when work session completes
      if (activeTaskId) {
        addTaskTime(activeTaskId, timerSetting.work * 60);
      }
    }
